    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <Link href={`/dashboard?ventureId=${ventureId}`} className="text-muted-foreground hover:text-foreground">
            ← Back
          </Link>
        ...
        <button
          ...
          className="rounded-lg bg-foreground px-4 py-2 text-sm font-medium text-background hover:bg-foreground"
        >
          Generate
        </button>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
              ...
              {list.map((d) => (
                <li key={d.id}>
                  <button
                    ...
                    className={`flex w-full flex-col items-start py-3 text-left text-sm hover:bg-background ${
                      selectedId === d.id ? "bg-background" : ""
                    }`}
                  >
        ...
        <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
          ...
          {!detail ? (
            <p className="mt-4 text-sm text-muted-foreground">Select a document to read.</p>
          ) : (
            <article className="mt-4 max-h-[70vh] overflow-y-auto whitespace-pre-wrap rounded-lg bg-background p-4 text-sm text-foreground">
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-overlay/40 p-4">
          <div className="w-full max-w-md rounded-xl bg-card p-6 shadow-xl">
            <h3 className="text-lg font-semibold text-foreground">Generate document</h3>
            <label htmlFor="doc-type" className="mt-4 block text-sm font-medium text-foreground">
              Type
            </label>
            <select
              id="doc-type"
            ...
            <div className="mt-6 flex justify-end gap-2">
              <button
                ...
                className="rounded-lg px-4 py-2 text-sm text-foreground hover:bg-muted"
              >
                Cancel
              </button>
              <button
