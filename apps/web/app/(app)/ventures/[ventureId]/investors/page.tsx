  ...
  return (
    <div className="space-y-10">
      <div>
        ...
        <h1 className="mt-2 text-2xl font-bold text-zinc-50">Investor relations</h1>
        <p className="text-sm text-zinc-400">Fundraising pipeline and investor updates</p>
      </div>

      <section>
        <h2 className="mb-3 text-lg font-semibold text-zinc-100">Pipeline</h2>
        <div className="flex gap-3 overflow-x-auto pb-2">
          {INVESTOR_STAGES.map((stage) => (
            <div
              key={stage}
              className="min-w-[200px] flex-1 rounded-xl border border-zinc-800 bg-zinc-900/50 p-2"
      ...
      <form onSubmit={addDeal} className="rounded-xl border border-amber-500/15 bg-zinc-900/60 p-4 space-y-3">
        <h2 className="font-semibold text-zinc-100">New investor deal</h2>
      ...
      {selected && (
        <div className="rounded-xl border border-amber-500/25 bg-zinc-900/80 p-6">
          ...
          <div className="mt-4 border-t border-zinc-800 pt-4">
            <h3 className="text-sm font-semibold text-amber-200/80">Interactions</h3>
      ...
      <section className="rounded-xl border border-amber-500/15 bg-zinc-900/60 p-6">
        <h2 className="text-lg font-semibold text-zinc-100">Investor update</h2>
      ...
      <section>
        <h2 className="mb-3 text-lg font-semibold text-zinc-100">Past updates</h2>
        <ul className="space-y-3">
          {updates.map((u) => (
            <li key={u.id} className="rounded-lg border border-zinc-800 bg-zinc-950/60 p-4">
              <p className="font-medium text-zinc-100">{u.title}</p>
              <p className="text-xs text-zinc-500">
                {viewerTz ? formatUserDateTime(u.createdAt, viewerTz) : u.createdAt}
              </p>
              <pre className="mt-2 max-h-48 overflow-auto whitespace-pre-wrap text-sm text-zinc-400">{u.body}</pre>
            </li>
          ))}
          {updates.length === 0 && <p className="text-sm text-zinc-600">No updates yet.</p>}
        </ul>
      </section>
    </div>
  );
