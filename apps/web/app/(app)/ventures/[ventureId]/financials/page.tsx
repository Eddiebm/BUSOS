    ...
    <div className="space-y-8">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <h1 className="mt-2 text-2xl font-bold tracking-tight text-zinc-50">Financial hub</h1>
          <p className="text-sm text-zinc-400">Cash flow, burn, and runway — timezone: {viewerTz ?? "…"}</p>
        </div>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="rounded-xl border border-amber-500/15 bg-zinc-900/60 p-4 lg:col-span-2">
          <h2 className="text-sm font-semibold uppercase tracking-wide text-amber-200/90">
            Income vs expense (by month)
          </h2>
          <div className="mt-4 flex h-48 items-end gap-1 border-b border-zinc-800 pb-2">
            {chart.last.length === 0 ? (
              <p className="text-sm text-zinc-500">Add transactions to see the chart.</p>
            ) : (
              chart.last.map((k) => {
                ...
                return (
                  <div key={k} className="flex min-w-0 flex-1 flex-col items-center gap-1">
                    ...
                    <span className="truncate text-[10px] text-zinc-500">{k.slice(2)}</span>
                  </div>
                );
              })
            )}
      ...
      <div className="rounded-xl border border-amber-500/15 bg-zinc-900/60 p-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h2 className="text-lg font-semibold text-zinc-100">Transactions</h2>
        ...
        <div className="mt-4 overflow-x-auto">
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {filtered.map((t) => (
                <tr key={t.id} className="border-b border-zinc-800/80 text-zinc-300">
          ...
          {filtered.length === 0 && <p className="mt-4 text-sm text-zinc-500">No transactions match.</p>}
        </div>
      </div>
    </div>
  );
...
function MetricCard({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl border border-amber-500/20 bg-gradient-to-br from-zinc-900 to-zinc-950 p-4">
      <p className="text-xs font-medium uppercase tracking-wide text-zinc-500">{label}</p>
      <p className="mt-1 text-2xl font-semibold text-amber-100">{value}</p>
    </div>
  );
}
  ...
  return (
    <div className="rounded-xl border border-dashed border-zinc-700 bg-zinc-900/40 p-4">
      <div className="flex items-center justify-between gap-2">
        ...
        <span className="rounded-full bg-zinc-800 px-2 py-0.5 text-xs text-amber-200/80">Coming soon</span>
      </div>
    </div>
  );
}
