    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100">
      <header className="sticky top-0 z-20 border-b border-zinc-800/80 bg-zinc-950/95 backdrop-blur">
        <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-4 px-4 py-4 sm:px-6">
          ...
          <a
            href={exportHref}
            className={cn(
              "inline-flex items-center gap-2 rounded-lg border border-amber-500/35 bg-amber-500/10 px-4 py-2 text-sm font-medium text-amber-100 transition hover:bg-amber-500/20",
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-8 px-4 py-8 lg:flex-row lg:gap-12 sm:px-6">
        <aside className="lg:w-48 lg:flex-shrink-0">
          <nav
            className="sticky top-24 space-y-1 rounded-xl border border-zinc-800 bg-zinc-900/50 p-3 text-sm lg:block"
          ...
          >
            ...
            {SECTIONS.map((s) => (
              <a
                key={s.id}
                href={`#${s.id}`}
                className="block rounded-md px-2 py-1.5 text-zinc-400 transition hover:bg-zinc-800 hover:text-amber-200/90"
              >
                {s.label}
              </a>
            ))}
          ...
          {!loading && !error && data && (
            <>
              <section id="overview" className="scroll-mt-28">
                <h2 className="mb-3 text-lg font-semibold text-amber-200/95">Overview</h2>
                <div className="rounded-xl border border-zinc-800 bg-zinc-900/40 p-6">
                  <h3 className="text-2xl font-semibold text-zinc-50">{data.ventureName}</h3>
                  {data.ventureDescription ? (
                    <p className="mt-3 text-sm leading-relaxed text-zinc-300">{data.ventureDescription}</p>
                  ) : (
                    <p className="mt-3 text-sm italic text-zinc-500">No description yet.</p>
                  )}
                  {data.dna && (
              ...
              <section id="value-prop" className="scroll-mt-28">
                <h2 className="mb-3 text-lg font-semibold text-amber-200/95">Value proposition</h2>
                <div className="rounded-xl border border-amber-500/30 bg-zinc-900/60 p-6">
                  {vpSentence ? (
                    <p className="text-sm leading-relaxed text-zinc-200">{vpSentence}</p>
                  ) : (
              ...
              <section id="competitors" className="scroll-mt-28">
                <h2 className="mb-3 text-lg font-semibold text-amber-200/95">Competitive landscape</h2>
                <div className="overflow-x-auto rounded-xl border border-zinc-800">
                  <table className="w-full min-w-[640px] border-collapse text-left text-sm">
                    <thead>
                      <tr className="border-b border-zinc-800 bg-zinc-900/80">
                        <th className="px-4 py-3 font-semibold text-amber-200/90">Competitor</th>
                        <th className="px-4 py-3 font-semibold text-amber-200/90">Pricing</th>
                        <th className="px-4 py-3 font-semibold text-amber-200/90">Strengths</th>
                        <th className="px-4 py-3 font-semibold text-amber-200/90">Weaknesses</th>
                      </tr>
                    </thead>
                    <tbody>
                      {data.competitors.length === 0 ? (
                        ...
                        data.competitors.map((c, i) => (
                          <tr key={i} className="border-b border-zinc-800/80">
                            ...
                            <td className="px-4 py-3 text-zinc-300">{c.pricing || "—"}</td>
                            <td className="px-4 py-3 text-zinc-300">{c.strengths || "—"}</td>
                            <td className="px-4 py-3 text-zinc-300">{c.weaknesses || "—"}</td>
                          </tr>
                        ))
                      )}
                    </tbody>
              ...
              <section id="mvp" className="scroll-mt-28">
                <h2 className="mb-3 text-lg font-semibold text-amber-200/95">MVP scope</h2>
                <div className="grid gap-6 md:grid-cols-2">
                  <div className="rounded-xl border border-zinc-800 bg-zinc-900/40 p-5">
                    <p className="mb-3 text-xs font-semibold uppercase tracking-wide text-emerald-400/90">
                      In MVP
                    </p>
              ...
              <section id="financial" className="scroll-mt-28">
                <h2 className="mb-3 text-lg font-semibold text-amber-200/95">Financial model</h2>
                {data.financialModel ? (
                  <div className="space-y-6">
                    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                      <div className="rounded-lg border border-zinc-800 bg-zinc-900/50 px-4 py-3">
                        <p className="text-xs text-zinc-500">Starting users (month 1)</p>
                        <p className="mt-1 font-medium text-zinc-100">
                          {fmtMoney(data.financialModel.startingUsers)}
                        </p>
                      </div>
                      <div className="rounded-lg border border-zinc-800 bg-zinc-900/50 px-4 py-3">
                        <p className="text-xs text-zinc-500">Revenue per user / month</p>
                        <p className="mt-1 font-medium text-zinc-100">
                          {fmtMoney(data.financialModel.revenuePerUser)}
                        </p>
                      </div>
                      <div className="rounded-lg border border-zinc-800 bg-zinc-900/50 px-4 py-3">
                        <p className="text-xs text-zinc-500">User growth % / month</p>
                        <p className="mt-1 font-medium text-zinc-100">
                          {data.financialModel.monthlyGrowthPct}%
                        </p>
                      </div>
                      <div className="rounded-lg border border-zinc-800 bg-zinc-900/50 px-4 py-3">
                        <p className="text-xs text-zinc-500">COGS % of revenue</p>
                        <p className="mt-1 font-medium text-zinc-100">{data.financialModel.cogsPct}%</p>
                      </div>
                      <div className="rounded-lg border border-zinc-800 bg-zinc-900/50 px-4 py-3">
                        <p className="text-xs text-zinc-500">Fixed costs / month</p>
                        <p className="mt-1 font-medium text-zinc-100">
                    ...
                    {data.financialSummary && (
                      <div className="rounded-xl border border-amber-500/25 bg-amber-500/5 p-6">
                        ...
                        <div className="mt-4 flex flex-wrap gap-8">
                          <div>
                            <p className="text-xs text-zinc-500">Year 1 revenue (sum)</p>
                            <p className="text-xl font-semibold text-zinc-50">
                              {fmtMoney(data.financialSummary.yearOneRevenue)}
                            </p>
                          </div>
                          <div>
                            <p className="text-xs text-zinc-500">Year 1 net profit (sum)</p>
                            <p
                              className={cn(
              ...
              <section id="team" className="scroll-mt-28">
                <h2 className="mb-3 text-lg font-semibold text-amber-200/95">Team</h2>
                <ul className="space-y-2 rounded-xl border border-zinc-800 bg-zinc-900/40 p-5">
                  {data.team.length === 0 ? (
                    <li className="text-sm text-zinc-500">No venture members listed yet.</li>
                  ) : (
                    data.team.map((t, i) => (
              ...
              <section id="progress" className="scroll-mt-28">
                <h2 className="mb-3 text-lg font-semibold text-amber-200/95">Progress</h2>
                <div className="rounded-xl border border-zinc-800 bg-zinc-900/40 p-6">
                  <div className="mb-2 flex justify-between text-sm">
                    <span className="text-zinc-300">
                      {data.completedMilestones} of {data.totalMilestones} milestones complete
                    </span>
                    <span className="text-amber-200/90">{progressPct}%</span>
                  </div>
                  <div className="h-3 overflow-hidden rounded-full bg-zinc-800">
                    <div
                      className="h-full rounded-full bg-amber-500/75 transition-all"
                      style={{ width: `${progressPct}%` }}
                    />
                  </div>
                  <p className="mt-4 text-xs text-zinc-500">
                    <Link href={`/ventures/${ventureId}/journey`} className="text-amber-400/90 hover:underline">
                      Open Journey
                    </Link>{" "}
